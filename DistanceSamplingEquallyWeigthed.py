...
# -*- coding: utf-8 -*-
import random
def calculateDiffernce(trace1,trace2,cells,maxActivities):
   difActivities=0
   difTime=0
   ...
   return round((difActivities/maxActivities + difTime/(len(trace1)/2))*100)
...
def findAllDistancesEquallyWeighted(results,statsTimes,maxActivities,percentOfData=0.1,numberOfCells=10):
    ...
    for i in range(len(results)):
        ...
        for j in range(len(results)):
            if j<=i:
                ...
                distances[i].append(calculateDiffernce(results[i],results[j],cells,maxActivities))
    return distances
