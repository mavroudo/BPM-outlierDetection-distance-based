...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 14:38:58 2020

@author: mavroudo
"""
from preprocess import dataPreprocess2017
from statistics import mean, stdev
from pm4py.objects.log.importer.xes import factory as xes_factory
from outlierDistanceActivities import createPairs,findOutlierEvents
...
@author: mavroudo
"""
import outlierPairsDistribution
import random 
#create outliers
...
def addOutliersInDataDistance(seq:list,dataVectors:list,outliers:list):
    """
...
for x,y in zip([2],[3]):
    ...
    outliersPositioned=addOutliersInDataDistance(seq,dataVectors,outliersCreated)   
    ...
    with open("resultsDistanceErrors17"+str(x)+"-"+str(y)+".txt","w") as f:
        for r in results:
    ...
    for t in thresholds:   
        print(x,y,t)
        foundOutliersCurve,distributions,means=outlierPairsDistribution.outlierDetectionWithDistribution(log,dataVectors,t)
    ...
    with open("resultsCurveErrors17"+str(x)+"-"+str(y)+".txt","w") as f:
        for r in results:
            f.write(str(r[0])+","+str(r[1])+","+str(r[2])+"\n")
...
for x,y in zip([2,3,4],[3,4,5]):
    results.append(readFile("resultsDistanceErrors17"+str(x)+"-"+str(y)+".txt"))
    results.append(readFile("resultsCurveErrors17"+str(x)+"-"+str(y)+".txt"))
