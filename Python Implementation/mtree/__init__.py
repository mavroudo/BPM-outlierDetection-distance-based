...
from mtree.heap_queue import HeapQueue
...
class _Node(_IndexItem):
	...
	def _check(self, mtree):
		...
		child_height = None
		for data, child in iter(self.children.items()):
...
class _NonLeafNodeTrait(_Node):
	...
	def add_child(self, new_child, distance, mtree):
		...
		while new_children:
			...
			if new_child.data not in self.children:
				...
				for grandchild in iter(new_child.children.values()):
	...
	def balance_children(self, the_child, mtree):
		...
		for another_child in (child for child in iter(self.children.values()) if child is not the_child):
		...
		if nearest_donor is None:
			# Merge
			for grandchild in iter(the_child.children.values()):
			...
			for grandchild in iter(nearest_donor.children.values()):
